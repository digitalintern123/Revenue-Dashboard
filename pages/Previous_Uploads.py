            ...
            if single_df.empty:
                ...
                try:
                    ...
                    if _aop is not None and not _aop.empty and "aop_target" in _aop.columns:
                        # revenue_master already carries an (often empty) `aop` column;
                        # drop it so the merge yields one AOP column, not aop_x/aop_y.
                        display_single = display_single.drop(columns=["aop"], errors="ignore").merge(
                ...
                if view_mode == "Daily totals":
                    ...
                    try:
                        ...
                        if _aop2 is not None and not _aop2.empty and "aop_target" in _aop2.columns:
                            dr = dr.drop(columns=["aop"], errors="ignore").merge(
    ...
    if not available_dates:
        ...
        if st.button("Load Selected Date", type="primary", icon=":material/event_available:"):
            ...
            st.success(
                ...
                f"Open **EHPL**, **Encalm Eats** or **Sky Plates** to explore it."
            )
