...
def _esc(value) -> str:
    """
    HTML-escape any string that comes from uploaded data (outlet names,
    locations, segments) before embedding in HTML rendered via
    unsafe_allow_html=True. Prevents XSS if a malicious Excel file
    contains outlet names like <script>alert(1)</script>.
    """
...
def _top_bottom_section(current_df: pd.DataFrame, compare_df: pd.DataFrame, n: int) -> Optional[str]:
    ...
    for _, row in top.head(n).iterrows():
        ...
        lines.append(
            f"- **{_esc(row['outlet'])}** ({_esc(row['location'])}): "
...
def _driver_section(current_df: pd.DataFrame, compare_df: pd.DataFrame) -> Optional[str]:
    ...
    if not volume_driven.empty:
        ...
        examples = ", ".join(f"{_esc(r['outlet'])} ({_esc(r['location'])})" for _, r in top_vol.iterrows())
    ...
    if not spend_driven.empty:
        ...
        examples = ", ".join(f"{_esc(r['outlet'])} ({_esc(r['location'])})" for _, r in top_spend.iterrows())
...
def _segment_summary_section(
    ...
    for _, row in seg_totals.iterrows():
        lines.append(f"- **{_esc(row['segment'])}**: {format_money(row['revenue'])} ({format_pax(row['pax'])} PAX)")
    ...
    if compare_df is not None and not compare_df.empty:
        ...
        if growth["top_growing"] is not None:
            ...
            lines.append(
                f"\n*vs {compare_label}:* "
                f"🔺 Fastest growing segment: **{_esc(g['segment'])}** ({_colored_pct(g['revenue_pct_change'])}). "
                f"🔻 Fastest declining: **{_esc(d['segment'])}** ({_colored_pct(d['revenue_pct_change'])})."
            )
    return "\n".join(lines)
