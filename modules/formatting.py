...
def _format_indian_smart(value: float) -> str:
    """
    Indian-grouped number that shows decimals only when the value actually
    has a fractional part — never a bare trailing ".0".
    e.g. 1250.0 -> "1,250"   1250.50 -> "1,250.50"   1250.567 -> "1,250.57"
    """
    v = round(float(value), 2)
    negative = v < 0
    v_abs = abs(v)
    whole = int(v_abs)
    frac = round(v_abs - whole, 2)
    grouped = _indian_group(whole)
    if frac == 0:
        result = grouped
    else:
        frac_str = f"{frac:.2f}".split(".")[1]
        result = f"{grouped}.{frac_str}"
    return f"-{result}" if negative else result


def format_pax(value: Number) -> str:
    """Indian-grouped PAX/count. Whole numbers show no decimals; a genuine
    fractional value keeps up to 2 decimal places. None/NaN -> em dash.
    e.g. 122385 → 1,22,385 · 1250.50 → 1,250.50"""
    if _is_missing(value):
        return EM_DASH
    return _format_indian_smart(value)


def format_money(value: Number) -> str:
    """₹-prefixed revenue, always a whole number (fractional paisa rounded).
    e.g. 25206553.0 -> Rs2,52,06,553  |  19304324.50 -> Rs1,93,04,325
    None/NaN/inf -> em dash. Underlying values unchanged."""
    if _is_missing(value):
        return EM_DASH
    if math.isinf(value):
        return EM_DASH
...
def format_spp(value: Number) -> str:
    """
    Intelligent SPP formatter (SPP = Revenue ÷ Terminal Traffic).
