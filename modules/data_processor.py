...
_MAX_FILE_BYTES = 50 * 1024 * 1024  # 50 MB — prevents zip-bomb / OOM


def _safe_exc_msg(exc: Exception) -> str:
    """
    Return a user-safe error message from an exception.
    SQLAlchemy exceptions include SQL statements + parameter values in their
    str() representation — these reveal DB schema and business data to users.
    Strip them and return a generic message with just the exception type.
    """
    name = type(exc).__name__
    msg = str(exc)
    # SQLAlchemy errors: truncate at first [SQL: or (Background on
    for marker in ["[SQL:", "[parameters:", "(Background on", "\nOriginal exception"]:
        idx = msg.find(marker)
        if idx > 0:
            msg = msg[:idx].strip()
    # If still long (> 200 chars) it probably has internal detail — truncate
    if len(msg) > 200:
        msg = msg[:200] + "…"
    return f"{name}: {msg}"

# Magic-byte signatures for binary formats. Text-based formats (CSV, TSV,
# TXT, HTML, XML, JSON) cannot be validated via magic bytes — they are
# validated by a UTF-8 decodability check instead (see _validate_upload).
# OLE2 magic covers .xls and .msg (Outlook messages); ZIP PK covers
# .xlsx/.xlsm/.docx (all Office Open XML formats).
_ALLOWED_MAGIC: dict[str, list[bytes]] = {
    ".xlsx": [bytes([0x50, 0x4B, 0x03, 0x04])],   # ZIP/PK
    ".xlsm": [bytes([0x50, 0x4B, 0x03, 0x04])],   # ZIP/PK
    ".xls":  [bytes([0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1])],  # OLE2
    ".pdf":  [b"%PDF"],
    ".docx": [bytes([0x50, 0x4B, 0x03, 0x04])],   # ZIP/PK
    ".msg":  [bytes([0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1])],  # OLE2
}

# Text-based extensions: validated by UTF-8 check, not magic bytes.
_TEXT_EXTENSIONS: frozenset[str] = frozenset({
    ".csv", ".tsv", ".txt", ".html", ".htm", ".xml", ".json",
})

# Image extensions validated by their own magic bytes.
...
_ALL_ALLOWED_EXTENSIONS: frozenset[str] = (
    frozenset(_ALLOWED_MAGIC) | _TEXT_EXTENSIONS | frozenset(_IMAGE_MAGIC)
)


def _validate_upload(file_obj, file_name: str) -> Optional[str]:
    """
    Server-side validation: size limit, extension whitelist, and either
    magic-byte check (binary formats) or UTF-8 decodability (text formats).
    Returns an error string on failure, None on success.

    Covers all file types offered by the file_uploader widget so that users
    never receive a confusing rejection for a format the UI advertised.
    """
    import os as _os
    file_obj.seek(0, 2)
    size = file_obj.tell()
    ...
    if size > _MAX_FILE_BYTES:
        return (
            f"File '{file_name}' is {size // (1024*1024)} MB — "
            f"maximum allowed is {_MAX_FILE_BYTES // (1024*1024)} MB."
        )
    ext = _os.path.splitext(file_name)[1].lower()
    if ext not in _ALL_ALLOWED_EXTENSIONS:
        friendly = ", ".join(sorted(_ALL_ALLOWED_EXTENSIONS))
        return (
    ...
    if ext in _IMAGE_MAGIC:
        ...
        file_obj.seek(0)
        if not any(header.startswith(sig) for sig in _IMAGE_MAGIC[ext]):
            return (
                f"'{file_name}' does not appear to be a valid {ext} image "
                f"(magic bytes mismatch)."
            )
        return None

    if ext in _TEXT_EXTENSIONS:
        # Validate text files by checking for non-text binary content.
        # Strategy: read up to 8 KB and:
        #   1. Reject if it fails UTF-8 AND latin-1 (true for some multibyte garbage).
        #   2. Reject if null bytes (0x00) make up more than 1% of the sample —
        #      legitimate CSV/JSON/HTML files never contain null bytes, but binary
        #      executables, ZIP files, and OLE2 containers do.
        sample = file_obj.read(8192)
        file_obj.seek(0)
        null_ratio = sample.count(b"\x00") / max(len(sample), 1)
        if null_ratio > 0.01:
            return (
                f"'{file_name}' appears to be a binary file, not a text file "
                f"(contains {null_ratio:.0%} null bytes). "
                f"Please upload a plain-text {ext} file."
            )
        try:
            sample.decode("utf-8")
            ...
            try:
                ...
                return (
                    f"'{file_name}' does not appear to be a valid text file "
                    f"(contains non-text binary data)."
                )
        return None
    ...
    header = file_obj.read(8)
    file_obj.seek(0)
    if not any(header.startswith(sig) for sig in _ALLOWED_MAGIC[ext]):
        return (
            f"'{file_name}' does not appear to be a valid {ext} file "
            f"(magic bytes mismatch). Please upload a genuine {ext} file."
        )
    return None
...
def process_uploaded_file(
    ...
    """
    # Sanitize filename: path traversal + markdown injection prevention
    import os as _os, re as _re
    file_name = _os.path.basename(str(file_name).strip().replace("\\", "/"))
    file_name = file_name if file_name else "uploaded_file"
    # Remove markdown special chars to prevent link injection in UI messages
    file_name = _re.sub(r"[\[\]()!`*_#|<>]", "", file_name) or "uploaded_file"

    # Validate before doing anything else
    val_err = _validate_upload(file_obj, file_name)
    if val_err:
        return ProcessResult(
            success=False, file_name=file_name, stage="validation", message=val_err
        )
    ...
    try:
        ...
        return ProcessResult(
            ...
            file_name=file_name,
            stage="reading",
            message=f"Unexpected error while processing '{file_name}': {_safe_exc_msg(exc)}",
        )

    _MAX_ROWS = 500_000
    if len(df) > _MAX_ROWS:
        return ProcessResult(
            success=False, file_name=file_name, stage="validation",
            message=f"File '{file_name}' has {len(df):,} rows — max allowed is {_MAX_ROWS:,}. Split into smaller files."
        )
    ...
    if save_to_db and "aop_daily" in df.columns and df["aop_daily"].notna().any():
        ...
        try:
            ...
            warnings.append(
                f"Found AOP figures embedded in this file, but saving them failed: {_safe_exc_msg(exc)} "
    ...
    if save_to_db:
        try:
            # Use batched save for large files (>5000 rows) to avoid
            # Neon PostgreSQL compute timeouts on free tier.
            if len(df) > 5000:
                result = database.save_dataframe_batched_v2(df, source_file=file_name)
            else:
            ...
            return ProcessResult(
                ...
                message=(
                    ...
                    f"saving it to the database failed: {_safe_exc_msg(exc)}"
                ),
            )
...
def _try_process_as_aop(file_obj, file_name: str, save_to_db: bool) -> ProcessResult:
    ...
    try:
        ...
        return ProcessResult(
            ...
            file_name=file_name,
            stage="reading",
            message=f"Unexpected error while reading '{file_name}' as an AOP workbook: {_safe_exc_msg(exc)}",
        )
    ...
    try:
        ...
        return ProcessResult(
            ...
            message=(
                ...
                f"but saving it to the database failed: {_safe_exc_msg(exc)}"
            ),
        )
...
def _try_process_as_traffic(file_obj, file_name: str, save_to_db: bool) -> ProcessResult:
    ...
    try:
        ...
        return ProcessResult(
            ...
            file_name=file_name,
            stage="reading",
            message=f"Unexpected error while reading '{file_name}' as traffic data: {_safe_exc_msg(exc)}",
        )
...
def _process_with_ingestion_layer(
    ...
    try:
        ...
        return ProcessResult(
            ...
            file_name=file_name,
            stage="reading",
            message=f"Ingestion layer error for '{file_name}': {_safe_exc_msg(exc)}",
        )
        ...
        try:
            ...
            return ProcessResult(
                ...
                message=(
                    ...
                    f"but saving to the database failed: {_safe_exc_msg(exc)}"
                ),
            )
...
def _process_with_universal(
    ...
    try:
        ...
        return ProcessResult(
            ...
            file_name=file_name,
            stage="reading",
            message=f"Unexpected error during automatic schema detection of '{file_name}': {_safe_exc_msg(exc)}",
        )
        ...
        try:
            ...
            return ProcessResult(
                ...
                message=(
                    ...
                    f"({len(df):,} row(s)), but saving it to the database failed: {_safe_exc_msg(exc)}"
                ),
            )
        inserted, skipped = result["inserted"], result["skipped"]
...
def _title_case_preserving_acronyms(value) -> str:
    """
    ...
    Handles None, pd.NA, float NaN, and the string "<NA>" that
    pd.NA produces after .astype(str) — all treated as empty string.
    """
    import pandas as pd
    if value is None:
        return ""
    ...
    if s.lower() in ("nan", "<na>", "none", ""):
        return ""
    if s.lower() in _KNOWN_ACRONYM_SEGMENTS:
        return s.upper()
    return s.title()
