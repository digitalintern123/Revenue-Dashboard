...
# RotatingFileHandler caps log growth at 10 MB × 5 backup files = 50 MB max.
# Without rotation, encalm_analytics.log would grow unbounded on a VPS.
# ---------------------------------------------------------------------------
...
if not _logger.handlers:
    ...
    _handler = _RotatingFileHandler(
        _os.path.join(_log_dir, "encalm_analytics.log"),
...
# Patterns that could expose secrets if logged from exception messages
_SECRET_PATTERNS = [
    "postgresql://", "postgres://", "mongodb://", "mysql://",
    "sqlite:///", "password=", "pwd=", "token=", "secret=",
    "apikey=", "api_key=", "ghp_", "sk-",
]


def _sanitize_for_log(text: str) -> str:
    """Redact any potential secrets from text before writing to log file."""
    lower = text.lower()
    for pattern in _SECRET_PATTERNS:
        if pattern.lower() in lower:
            # Redact the entire line containing the sensitive pattern
            lines = text.split("\n")
            text = "\n".join(
                "[REDACTED - potential secret]" if pattern.lower() in line.lower() else line
                for line in lines
            )
            break
    return text
...
def log_exception(
    ...
    Sensitive patterns (DB URLs, tokens) are automatically redacted.
    """
    ...
    _logger.error(_sanitize_for_log(raw))
