...
_COMBINE_INTO_TARGET = {
    # "M&G" (plain) maps to Atithya for all locations.
    # "M&G Hyd" is the Atithya AOP for Hyderabad (not a subtotal — Porter/Buggy are 0).
    # "M&G Del" IS a subtotal (M&G + Porter Del + Buggy Del) — skip it.
    # "M&G Goa" IS a subtotal — skip it.
    ("Delhi",     "Atithya"): ["M&G"],
    ("Hyderabad", "Atithya"): ["M&G", "M&G Hyd"],
    ("Goa",       "Atithya"): ["M&G"],
}

# These are subtotal rows in the AOP file — skip them to avoid double-counting
_AOP_SUBTOTAL_UNIT_IDS = frozenset([
    "M&G Del", "M&G Goa",   # Delhi and Goa subtotals only; M&G Hyd is NOT a subtotal
])

# Standalone Porter and Buggy outlet name normalization
_AOP_ATITHYA_MAP = {
    ("Delhi",     "Porter Del"):  "Porter",
    ("Delhi",     "Buggy Del"):   "Buggy",
    ("Hyderabad", "Porter Hyd"):  "Porter",
    ("Hyderabad", "Buggy Hyd"):   "Buggy",
    ("Hyderabad", "GAT"):         "Atithya",   # Hyderabad: GAT = Meet & Greet / Atithya
    ("Goa",       "Porter Goa"):  "Porter",
    ("Goa",       "Buggy Goa"):   "Buggy",
    ("Goa",       "GAT"):         "Atithya",   # Goa: GAT = Meet & Greet / Atithya
}
...
_AOP_OUTLET_MAP = {
    ...
    ("Hyderabad", "Amex Hyd"): "International Lounge (New)",
...
def parse_outlet_monthly_aop(file_obj, sheet_name: Optional[str] = None) -> dict:
    ...
    for row_idx in range(header_row_idx + 1, raw.shape[0]):
        ...
        # Skip Atithya subtotal rows (M&G Del / M&G Hyd / M&G Goa) — these are
        # location-level subtotals that equal M&G + Porter + Buggy combined.
        # Using them would double-count Atithya AOP.
        if unit_id in _AOP_SUBTOTAL_UNIT_IDS:
            continue
        ...
        # Porter Del / Buggy Del -> canonical Porter / Buggy
        atithya_key = (location, unit_id)
        if atithya_key in _AOP_ATITHYA_MAP:
            mapped_outlet = _AOP_ATITHYA_MAP[atithya_key]
            for (y, m), val in monthly_values.items():
                if val is None:
                    continue
                aop_records.append(
...
def _extract_month_columns(raw: pd.DataFrame, header_row_idx: int) -> list:
    ...
    for col_idx in range(5, len(header_row)):
        ...
        # Non-datetime, non-null cells are FY-total / summary columns
        # (e.g. "FY\n25-26", "FY\n26-27") — skip them instead of breaking
        # so that month columns spanning multiple fiscal years are all captured.
    return result
