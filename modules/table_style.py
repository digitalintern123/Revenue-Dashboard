    ...
    * **Location/segment-level table** (no "outlet" column):
    ...
    def _outlet_pen_spp(raw_df, joined_with_traffic):
        ...
        Per Business Plan:
          ...
          (SPP = Revenue ÷ Traffic, the standard airport-retail metric)
    ...
    if outlet_grain:
        # Merge traffic on (outlet, location) only — NOT segment.
        # Segment names differ between raw_df (old DB3: "Lounges"/"Atithya")
        # and comparison_df (normalised: "EHPL"/"Sky Plates"). Including segment
        # in merge_keys causes no matches → Traffic/PEN shows — for all outlets.
        merge_keys = ["outlet", "location"]
        ...
        def _outlet_traffic_pen_spp(raw_df):
            ...
            # Do NOT merge segment back — merge_keys is (outlet, location) only.
            # Merging segment back would reintroduce old segment values ("Lounges")
            # that don't match the normalised values in comparison_df ("EHPL").
            keep = [c for c in ["outlet", "location", "traffic", "penetration_pct", "spp"] if c in ot.columns]
            return ot[keep]
...
def add_aop_columns(
    ...
    # Filter out __location_total__ sentinel rows produced when only
    # daily-location-level AOP data is available. Per-outlet AOP columns
    # can only be shown when actual per-outlet targets exist in the
    # aop_target table — location-level totals are displayed separately
    # on the Executive Summary page and not as outlet-level variance here.
    if "outlet" in joined.columns:
        joined = joined[joined["outlet"] != "__location_total__"]

    # Drop aop_source sentinel column if present (from daily-source branch)
    joined = joined.drop(columns=["aop_source"], errors="ignore")

    if joined.empty or "aop_target" not in joined.columns or joined["aop_target"].isna().all():
        return comparison_df, []
    ...
    # Merge on (outlet, location) only when outlet-level — NOT segment.
    # comparison_df carries normalised segments ("EHPL"/"Sky Plates") while
    # `variance` derives segment from current_raw_df, which holds the raw
    # database values ("Lounges"/"Atithya"). Including segment in the merge
    # key makes every row miss and AOP renders as "—" for every outlet.
    if "outlet" in group_cols:
        _merge_on = [c for c in ["outlet", "location"] if c in variance.columns
                     and c in comparison_df.columns]
        variance = variance.drop(columns=["segment"], errors="ignore")
    else:
        _merge_on = group_cols

    out = comparison_df.merge(variance, on=_merge_on, how="left")
