...
# Sentinel returned by classify_trend_with_new() when an outlet had zero
# revenue in the compare period but positive revenue in the current period.
# Pages can check for this string to render "New" with distinct styling.
NEW_ENTRANT_TREND = "🆕 New"
...
def classify_trend(pct_change: Optional[float], metric_label: str = "Revenue") -> str:
    ...
    Note: when pct_change is None because previous==0, this returns
    "➡️ Stable" — which is misleading for a new-entrant outlet. Use
    classify_trend_with_new() in compare_periods() to handle that case.
    """
...
def classify_trend_with_new(
    pct_change_val: Optional[float],
    current_value: float,
    compare_value: float,
    metric_label: str = "Revenue",
) -> str:
    """
    Like classify_trend() but correctly handles the new-entrant case:
    if compare_value == 0 and current_value > 0, returns NEW_ENTRANT_TREND
    ("🆕 New") instead of "➡️ Stable", so newly opened outlets are
    visually distinguished from outlets that genuinely had flat performance.

    Also handles the exit case: if compare_value > 0 and current_value == 0
    returns "📉 Revenue Decline" (100% drop) rather than "—".
    """
    if compare_value == 0 and current_value > 0:
        return NEW_ENTRANT_TREND
    return classify_trend(pct_change_val, metric_label)
...
def pct_change(current: Optional[float], previous: Optional[float]) -> Optional[float]:
    ...
    -1.0 (-100%) for a new-negative when the base is zero, which:
      ...
      - Keeps classify_trend() working correctly (anything > GROWTH_THRESHOLD
    ...
    if previous == 0:
        ...
        # If previous is 0 (no data on one side), return None so the UI
        # shows "—" instead of a misleading percentage.
        # Callers that need to distinguish the new-entrant case (prev=0,
        # cur>0) from "genuinely no data" should check compare_revenue==0
        # directly on the comparison DataFrame — see classify_trend_with_new()
        # below, which is used by compare_periods() for the trend column.
        return None
...
def compare_periods(
    ...
    """
    Build a comparison table between two already-filtered DataFrames.

    group_cols controls the aggregation grain:
      - default GROUP_COLS ["segment","outlet","location"] → outlet-level
      - ["location","segment"] → location+segment summary (Tab 3)
      - ["segment"] → segment-only summary
      - ["location"] → location-only summary

    For outlet-level comparisons (group_cols contains "outlet"):
      Segment is EXCLUDED from the merge key to prevent duplicate rows when
      comparing old data (segment="Lounges") vs new data (segment="EHPL").
      Instead, segment is resolved from the raw DataFrames and normalised to EHPL.

    For non-outlet comparisons (location/segment summaries):
      group_cols is used as-is for both aggregation and merge.
    """
    # ── Legacy segment normalisation ─────────────────────────────────────
    _LEGACY = {"lounges", "atithya", "others", "subsidiary", "lounge"}

    def _norm_seg(s):
        if pd.isna(s) or str(s).strip() == "":
            return "EHPL"
        return "EHPL" if str(s).strip().lower() in _LEGACY else str(s).strip()

    # ── Normalise segment in raw DataFrames before aggregation ───────────
    def _prep(df):
        if df is None or df.empty:
            return df
        d = df.copy()
        if "segment" in d.columns:
            d["segment"] = d["segment"].apply(_norm_seg)
        return d

    current_df  = _prep(current_df)
    compare_df  = _prep(compare_df)

    # ── Outlet-level comparison (default) ────────────────────────────────
    if "outlet" in group_cols:
        # Always merge on (outlet, location) only — never segment
        # so old-segment rows and new-segment rows for the same outlet merge correctly
        join_cols = ["outlet", "location"]

        # Aggregate on outlet+location (sum across all segment values per outlet)
        current_agg = _aggregate(current_df, join_cols, suffix="current")
        compare_agg = _aggregate(compare_df, join_cols, suffix="compare")

        merged = pd.merge(current_agg, compare_agg, on=join_cols, how="outer")

        # Restore segment from raw data (one canonical value per outlet+location)
        def _seg_lookup(df):
            if df is None or df.empty or "segment" not in df.columns:
                return pd.DataFrame(columns=join_cols + ["segment"])
            return (
                df[join_cols + ["segment"]]
                .drop_duplicates(subset=join_cols, keep="first")
                .reset_index(drop=True)
            )

        seg_cur = _seg_lookup(current_df).rename(columns={"segment": "_sc"})
        seg_cmp = _seg_lookup(compare_df).rename(columns={"segment": "_sp"})
        merged  = merged.merge(seg_cur, on=join_cols, how="left")
        merged  = merged.merge(seg_cmp, on=join_cols, how="left")
        merged["segment"] = merged["_sc"].combine_first(merged["_sp"]).fillna("EHPL")
        merged  = merged.drop(columns=["_sc", "_sp"], errors="ignore")

    # ── Non-outlet comparisons (location/segment summaries) ──────────────
    else:
        ...
        # Normalise segment column if present in output
        if "segment" in merged.columns:
            merged["segment"] = merged["segment"].apply(_norm_seg)
    ...
    # Use classify_trend_with_new so outlets that appear in the current period
    # for the first time (compare_revenue == 0, current_revenue > 0) show
    # "🆕 New" instead of the misleading "➡️ Stable".
    merged["revenue_trend"] = merged.apply(
        lambda r: classify_trend_with_new(
            r["revenue_pct_change"], r["current_revenue"], r["compare_revenue"]
        ),
        axis=1,
    )
    ...
    merged["pax_trend"] = merged.apply(
        lambda r: classify_trend_with_new(
            r["pax_pct_change"], r["current_pax"], r["compare_pax"], "PAX"
        ),
        axis=1,
    )
...
def revenue_per_pax_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = df.copy()
    # Normalise legacy segment values so the table always shows EHPL/Sky Plates/Encalm Eats
    _LEGACY = {"lounges", "atithya", "others", "subsidiary", "lounge"}
    if "segment" in work.columns:
        work["segment"] = work["segment"].apply(
            lambda s: "EHPL" if (pd.isna(s) or str(s).strip().lower() in _LEGACY or str(s).strip() == "")
            else str(s).strip()
        )
    agg = work.groupby(["outlet", "location"], as_index=False).agg(
        revenue=("revenue", "sum"), pax=("pax", "sum")
    )
    # Attach normalised segment (one canonical value per outlet+location)
    seg_map = work[["outlet", "location", "segment"]].drop_duplicates(
        subset=["outlet", "location"], keep="first"
    )
    agg = agg.merge(seg_map, on=["outlet", "location"], how="left")
    agg["segment"] = agg["segment"].fillna("EHPL")
...
def spp(revenue: float, traffic: float) -> float:
    ...
    SPP = Outlet Revenue / Terminal Traffic for that outlet's assigned
    terminal pool. This is the standard airport-retail yield metric:
    how much revenue each airport visitor (not just Encalm customers)
    generates on average for that terminal.

    Used consistently across all pages and the Business Performance MIS:
      PEN % = PAX / Traffic × 100
      SPP   = Revenue / Traffic

    NaN if traffic is missing or zero.
    """
...
def location_level_summary_with_traffic(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # SPP = Revenue / Traffic (airport-retail yield per terminal visitor).
    # Consistent with the Business Performance MIS report and all page captions.
...
def explain_revenue_driver(
    ...
    key drivers of airport-retail revenue:
      Traffic      = airport visitors passing through that terminal
      Penetration% = PAX / Traffic × 100  (Encalm customers as % of visitors)
      SPP          = Revenue / Traffic     (revenue per airport visitor)

    Note: Revenue = PAX × (Revenue/PAX), not Traffic × Pen% × SPP — the
    three factors are used for driver attribution, not strict algebraic identity.
    ...
    revenue_up = revenue_pct > GROWTH_THRESHOLD
    revenue_down = revenue_pct < DECLINE_THRESHOLD
    ...
    traffic_up = traffic_known and traffic_pct > GROWTH_THRESHOLD
    traffic_down = traffic_known and traffic_pct < DECLINE_THRESHOLD
    pen_up = pen_known and penetration_pct_change > GROWTH_THRESHOLD
    pen_down = pen_known and penetration_pct_change < DECLINE_THRESHOLD
    spp_up = spp_known and spp_pct_change > GROWTH_THRESHOLD
    spp_down = spp_known and spp_pct_change < DECLINE_THRESHOLD
...
def explain_penetration_driver(
    ...
    pen_up = penetration_pct_change > GROWTH_THRESHOLD
    pen_down = penetration_pct_change < DECLINE_THRESHOLD
